(() => {
    ...
    const TEST_SELECTION_KEY = "note_summer_bundle_test_v3";
    ...
    function getCountdownEnd() {
        ...
        try {
            const stored = Number(window.NoteStore.session.getItem(COUNTDOWN_KEY));
            if (Number.isFinite(stored) && stored > Date.now()) return stored;
            window.NoteStore.session.setItem(COUNTDOWN_KEY, String(defaultEnd));
        } catch (_error) {
            return defaultEnd;
        }
    ...
    function getToast() {
        ...
        toast.className = "summer-test-toast";
        toast.dataset.summerToast = "";
        toast.setAttribute("role", "status");
        toast.setAttribute("aria-live", "polite");
        toast.innerHTML = `
            <strong>Testmodus aktiv.</strong>
            <span>Diese Aktion verändert keine Bestellung und startet keinen Checkout.</span>
        `;
    ...
    function readSavedSelection() {
        try {
            const stored = JSON.parse(window.NoteStore.local.getItem(TEST_SELECTION_KEY) || "{}");
    ...
    function initInlineBundleBuilder() {
        ...
        submitButton?.addEventListener("click", () => {
            ...
            try {
                window.NoteStore.local.setItem(TEST_SELECTION_KEY, JSON.stringify(selection));
            } catch (_error) {
                // The linked test page still opens if browser storage is unavailable.
            }

            window.location.href = "sommerbundle-test.html";
        });
    ...
    function readCachedProducts() {
        try {
            const cached = JSON.parse(window.NoteStore.session.getItem("note_products_v2") || "[]");
    ...
    async function loadBundleProducts() {
        ...
        for (const endpoint of endpoints) {
            try {
                ...
                try {
                    window.NoteStore.session.setItem("note_products_v2", JSON.stringify(products));
                } catch (_error) {
    ...
    function initBundleBuilder() {
        ...
        const renderProductList = () => {
            ...
            if (resultCount) {
                resultCount.textContent = isFallbackCatalog
                    ? `${matches.length} Düfte in der Testauswahl`
                    : matches.length
        ...
        submitButton?.addEventListener("click", () => {
            ...
            try {
                ...
                // The visual confirmation still works when storage is unavailable.
            }

            showToast(
                "Dein Test-Trio ist vorgemerkt.",
                "Die Auswahl wurde nur lokal gespeichert. Warenkorb und Checkout bleiben unverändert."
            );
        });
    ...
    function guardLiveCheckoutOnHomepageClone() {
        if (!document.body.classList.contains("summer-bundle-test-page")) return;

        const rewriteCloneLinks = () => {
            document.querySelectorAll('a[href="index.html#bestseller"]').forEach((link) => {
                link.setAttribute("href", "index-sommerbundle-test.html#bestseller");
            });
        };

        rewriteCloneLinks();
        const cloneLinkObserver = new MutationObserver(rewriteCloneLinks);
        cloneLinkObserver.observe(document.body, { childList: true, subtree: true });
        ...
        document.addEventListener("click", (event) => {
            ...
            showToast(
                "Checkout in der Testansicht deaktiviert.",
                "Zum Prüfen des Live-Checkouts bitte die reguläre Startseite verwenden."
            );
        }, true);
    }
    ...
    initBundleBuilder();
    guardLiveCheckoutOnHomepageClone();
})();
