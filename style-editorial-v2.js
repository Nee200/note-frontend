...
async function getCsrfToken() {
    const response = await window.NoteApi.fetch(`${API_BASE_URL}/api/csrf-token`, { credentials: "include" });
...
const editorialHero = document.querySelector("[data-hero-editorial]");

if (editorialHero) {
    window.requestAnimationFrame(() => editorialHero.classList.add("is-ready"));
    ...
    if (!prefersReducedMotion) {
        editorialHero.addEventListener("pointermove", (event) => {
            ...
            editorialHero.style.setProperty("--hero-shift-x", `${(x * 10).toFixed(2)}px`);
            editorialHero.style.setProperty("--hero-shift-y", `${(y * 7).toFixed(2)}px`);
        });

        editorialHero.addEventListener("pointerleave", () => {
            editorialHero.style.setProperty("--hero-shift-x", "0px");
            editorialHero.style.setProperty("--hero-shift-y", "0px");
        });

        let heroScrollFrame = 0;
        const updateEditorialHeroScroll = () => {
            heroScrollFrame = 0;
            const rect = editorialHero.getBoundingClientRect();
            const progress = Math.max(0, Math.min(-rect.top / Math.max(rect.height, 1), 1));
            editorialHero.style.setProperty("--hero-scale", (1.035 + progress * 0.045).toFixed(4));
            editorialHero.style.setProperty("--hero-copy-shift", `${(-progress * 20).toFixed(2)}px`);
        };
        const requestEditorialHeroScroll = () => {
            if (heroScrollFrame) return;
            heroScrollFrame = window.requestAnimationFrame(updateEditorialHeroScroll);
        };

        updateEditorialHeroScroll();
        window.addEventListener("scroll", requestEditorialHeroScroll, { passive: true });
        window.addEventListener("resize", requestEditorialHeroScroll);
    }
}
